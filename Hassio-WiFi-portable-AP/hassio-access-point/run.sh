...
# by LZ7AA, modified to work without LAN
...
DEFAULT_INTERFACE=$(bashio::config 'default_interface' )
...
logger "DEFAULT_ROUTE_INTERFACE=$DEFAULT_ROUTE_INTERFACE" 1
if [ ${#DEFAULT_ROUTE_INTERFACE} -eq 0 ]; then
    logger "#= NO default route!" 0
fi
...
if $(bashio::config.true "dhcp"); then
    ...
    ## DNS
    ## Preload dnsarray with defaults in case LAN interface is not available - portable (off-grid, stand-alone) use case
    dns_array=(8.8.8.8 1.1.1.1)
        if [ ${#CLIENT_DNS_OVERRIDE} -ge 1 ]; then
            ...
            if [ ${#DEFAULT_ROUTE_INTERFACE} -ne 0 ]; then
...
# Set HA configured default interface in case there is none (portable operation)
if [ ${#DEFAULT_ROUTE_INTERFACE} -eq 0 ]; then
...
if $(bashio::config.true "client_internet_access"); then
    ...
    ## Route traffic
    logger "# NFT masquerade ON $DEFAULT_ROUTE_INTERFACE" 1
